...
If the user input workflows that are not pre-defined, for example `discard uncommitted changes` our intent handler will direct the prompt to LLM, in our case, llama3, to generate a workflow, which the user can then review and confirm. The learned workflows are saved and can be triggered directly without go through the llm, ensures the speed.
